...
describe("= over the data: the whole word, and != its negation", () => {
  ...
  it("tells the substring from the complete word in rules text", () => {
    // Polar Bears reads "Submerge", which holds "merge" but not the word.
    expect(names("r:merge")).toEqual(["Polar Bears"]);
    expect(names("r=merge")).toEqual([]);
    expect(names("r=submerge")).toEqual(["Polar Bears"]);
  });
  it("negates the same question it asks, which : and - do not", () => {
    // != is the negation of =, so it keeps the card whose text holds
    // merge only inside a longer word. -r: is the negation of r:, so it
    // drops that card. Two different questions, each answered honestly.
    const all = ["Apprentice Wizard", "Broken Site", "Druid", "Polar Bears", "Witch"];
    expect(names("r!=merge")).toEqual(all);        // no card holds the word
    expect(names("-r=merge")).toEqual(all);        // the same question, same answer
    expect(names("-r:merge")).toEqual(["Apprentice Wizard", "Broken Site", "Druid", "Witch"]);
  });
  it("reads == and !== as = and !=, never as a value beginning with =", () => {
    expect(names("r==merge")).toEqual(names("r=merge"));
    expect(names("r!==merge")).toEqual(names("r!=merge"));
  });
  ...
  it("leaves the exact-name search meaning the whole name", () => {
    expect(names('!"Polar Bears"')).toEqual(["Polar Bears"]);
    expect(names('!"Polar"')).toEqual([]);        // the entire name
    expect(names("n=polar")).toEqual(["Polar Bears"]);  // a word inside it
  });
  ...
  it("still means equality on the kinds that had it, unchanged", () => {
    expect(names("m=3")).toEqual(names("m:3"));
    expect(names("t=minion")).toEqual(names("t:minion"));
    expect(names("e=water")).toEqual(["Polar Bears"]);   // exactly Water
    expect(names("e:water")).toEqual(["Polar Bears", "Witch"]);
  });
